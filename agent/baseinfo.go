...
func (b *BaseInfo) IsBox() bool {
	if os.Getenv("AGENT_IS_BOX") == "true" {
		return true
	}
	return isBox
}
...
func (baseInfo *BaseInfo) ToLuaTable(L *lua.LState) *lua.LTable {
	...
	t.RawSet(lua.LString("token"), lua.LString(baseInfo.token))
	t.RawSet(lua.LString("isBox"), lua.LBool(baseInfo.IsBox()))
	return t
}
...
func (b *BaseInfo) SetToken(token string) {
	b.token = token
}

func (b *BaseInfo) GetToken() string {
	return b.token
}
