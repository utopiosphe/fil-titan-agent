...
build_and_package() {
  ...
  if [[ -n "$TAG" ]]; then
    ...
    CGO_ENABLED=1 GOOS=android GOARCH=arm64 GOARM=8 go build -trimpath -tags "$TAG" || exit 1
    output_name="${output_name}-${TAG}"  # 在 zip 文件名中加上 tag
  else
    # CGO_ENABLED=1 GOOS=android GOARCH=arm GOARM=7 go build -trimpath || exit 1
    CGO_ENABLED=1 GOOS=android GOARCH=arm64 GOARM=8 go build -trimpath || exit 1
  fi
