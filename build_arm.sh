...
build_and_package() {
  ...
  # 编译二进制文件
  GOOS=linux GOARCH=arm go build -trimpath|| { echo "Build failed for $component_dir"; exit 1; }
...
# 编译并打包 agent
build_and_package "$AGENT_DIR" "agent-arm"

# 编译并打包 controller
build_and_package "$CONTROLLER_DIR" "controller-arm"

# 计算并显示 MD5 校验值
md5sum "$RELEASE_DIR/agent-arm.zip"
md5sum "$RELEASE_DIR/controller-arm.zip"
